	...
	Convey("CVE Pagination", t, func() {
		...
		Convey("Page", func() {
			Convey("defaults", func() {
				...
				cves, counters, pageInfo, err := cveInfo.GetCVEListForImage(ctx, "repo1", "0.1.0", "", cvemodel.PageInput{})
				So(err, ShouldBeNil)
				...
				cves, counters, pageInfo, err = cveInfo.GetCVEListForImage(ctx, "repo1", "1.0.0", "", cvemodel.PageInput{})
				So(err, ShouldBeNil)
			...
			Convey("no limit or offset", func() {
				...
				cves, counters, pageInfo, err := cveInfo.GetCVEListForImage(ctx, "repo1", "0.1.0", "",
				...
				sort.Strings(cveIds)
				cves, counters, pageInfo, err = cveInfo.GetCVEListForImage(ctx, "repo1", "1.0.0", "",
					cvemodel.PageInput{SortBy: cveinfo.AlphabeticAsc})
				So(err, ShouldBeNil)
				So(len(cves), ShouldEqual, 30)
				So(len(counters), ShouldEqual, 4)
